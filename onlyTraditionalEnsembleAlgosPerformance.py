...
import time
# dataset=pd.read_csv("/home/safia/drebinDataset/DatasetsFOrFlowAnalysis/labeledDatasetMalTraditional.csv") #remember to remove the first column
dataset = pd.read_csv("D://Research/Ransomware/safiasBackup/drebinDataset/DatasetsFOrFlowAnalysis/onlyTraditionalDataset.csv")  # remember to remove the first column
...
ROCtrainTRN, ROCtestTRN, ROCtrainTRG, ROCtestTRG = model_selection.train_test_split(numeric_data_best, y,
                                                                                    test_size=0.25)
results = {}
kfold = 5
...
roc_train_all, roc_test_all, roc_train_all_class, roc_test_all_class = model_selection.train_test_split(X, y,
                                                                                                        test_size=0.25)
roc_train_best, roc_test_best, roc_train_best_class, roc_test_best_class = model_selection.train_test_split(
    numeric_data_best, y, test_size=0.25)
...
models_box.append(
    ('AB', AdaBoostClassifier(DecisionTreeClassifier(max_depth=19), algorithm="SAMME.R", n_estimators=1000)))
models_box.append(('ETC', ExtraTreesClassifier(max_features='auto', criterion='entropy', n_estimators=1000)))
models_box.append(
    ('BC', BaggingClassifier(RandomForestClassifier(n_estimators=100, random_state=40), n_estimators=100)))
models_box.append(
    ('VC', VotingClassifier(estimators=[('xgboost', model_xgb), ('rf', model_rfc), ('gnb', model_gb)], voting='soft')))
seed = 7
results_box = []
names = []
scoring = 'accuracy'
